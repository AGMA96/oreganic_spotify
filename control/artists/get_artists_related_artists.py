...
LIKED_ARTISTS_PATH = os.path.normpath(os.path.join(os.getcwd(),'..', 'input', 'liked_artists_ids.json'))
...
RELATED_ARTISTS_PATHS = glob.glob(os.path.join(os.getcwd(), '*.json'))
...
for artist_id in target_artists.keys():
    ...
    with open(os.path.join(os.getcwd(), artist_id + '.json'), 'w', encoding='utf-8') as related_artists_file:
